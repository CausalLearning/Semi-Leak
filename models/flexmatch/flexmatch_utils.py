...
def consistency_loss(logits_s, logits_w, class_acc, p_target, p_model, name='ce',
    ...
    if name == 'L2':
        ...
        if use_DA:
            ...
            pseudo_label = (
                pseudo_label / pseudo_label.sum(dim=-1, keepdim=True))
        ...
        mask = max_probs.ge(
            p_cutoff * (class_acc[max_idx] / (2. - class_acc[max_idx]))).float()  # convex
        # mask = max_probs.ge(p_cutoff * (torch.log(class_acc[max_idx] + 1.) + 0.5)/(math.log(2) + 0.5)).float()  # concave
        select = max_probs.ge(p_cutoff).long()
        if use_hard_labels:
            masked_loss = ce_loss(
                logits_s, max_idx, use_hard_labels, reduction='none') * mask
        else:
