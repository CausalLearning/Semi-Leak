    ...
    def train(self, args, logger=None):
        ...
        for (_, x_lb, y_lb), (x_ulb_idx, x_ulb_w, x_ulb_s) in zip(self.loader_dict['train_lb'],
            ...
            with amp_cm():
                ...
                sup_mask = torch.max(torch.softmax(
                    logits_x_lb, dim=-1), dim=-1)[0].le(tsa).float().detach()

                sup_loss = (ce_loss(logits_x_lb, y_lb,
                            reduction='none') * sup_mask).mean()
            ...
            iter_per_epoch = 1024
            epoch = self.it // iter_per_epoch

            if self.it % (iter_per_epoch * args.log_every_epoch) == 0:
                print("==========Epoch %d===========" % (epoch))
                eval_dict = self.evaluate(args=args)
                ...
                if not args.multiprocessing_distributed or \
                    ...
                    if self.it == best_it:
                        self.save_model('%s_model_best.pth' %
                                        (args.mode), save_path)
                ...
                if epoch % args.save_every_epoch == 0:
                    ...
                    if not args.multiprocessing_distributed or \
                            (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0):
                        self.save_model('%s_model_%d.pth' %
                                        (args.mode, epoch), save_path)
            ...
            if self.it % 10000 == 0:
                ...
                if not args.multiprocessing_distributed or \
                        (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0):
                    self.save_model('latest_%s_model.pth' %
                                    (args.mode), save_path)
