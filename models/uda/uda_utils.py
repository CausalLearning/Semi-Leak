...
def consistency_loss(logits_s, logits_w, class_acc, it, ds, name='ce', T=1.0, p_cutoff=0.0, use_flex=False):
    ...
    if name == 'ce':
        ...
        if use_flex:
            mask = max_probs.ge(
                p_cutoff * (class_acc[max_idx] / (2. - class_acc[max_idx]))).float()
        else:
    ...
    if name == 'kld_tf':
        ...
        kld = F.kl_div(torch.log_softmax(logits_s, dim=-1),
                       torch.softmax(logits_tgt, dim=-1), reduction='none')
        masked_loss = kld * \
            loss_mask.unsqueeze(dim=-1).repeat(1, pseudo_labels.shape[1])
