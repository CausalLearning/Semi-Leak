...
def main_worker(gpu, ngpus_per_node, args):
    ...
    s = SSLDataset(args, alg='uda', name=args.dataset,
    ...
    if args.mode == "target":
        lb_dset = target_train_lb
        ulb_dset = target_train_ulb
        eval_dset = target_test
    elif args.mode == "shadow":
        lb_dset = shadow_train_lb
        ulb_dset = shadow_train_ulb
        eval_dset = shadow_test
    else:
        raise ValueError("mode should be either target or shadow")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_name', type=str, default='uda')
    ...
    parser.add_argument('--TSA_schedule', type=str,
                        default='none', help='TSA mode: none, linear, log, exp')
    parser.add_argument('--T', type=float, default=0.4,
                        help='Temperature sharpening')
    parser.add_argument('--ema_m', type=float, default=0.999,
                        help='ema momentum for eval_model')
    parser.add_argument('--ulb_loss_ratio', type=float, default=1.0)
    parser.add_argument('--p_cutoff', type=float, default=0.8,
                        help='confidencial masking threshold')
